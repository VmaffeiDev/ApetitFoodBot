...
def save_client(telegram_id: int, chat_id: int, name: str, phone: str, address: str, restriction: str, goal: str = "Nao informado", consent: bool = False, consented_at: str | None = None) -> None:
    timestamp = now_iso()
    with db() as conn:
        columns = {row["name"] for row in conn.execute("PRAGMA table_info(clients)").fetchall()}
...
async def handle_menu_csv_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
def main() -> None:
    ...
    app.add_handler(MessageHandler(filters.Document.ALL, handle_menu_csv_upload))
